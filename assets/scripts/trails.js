...
C=document.layers;
T1=new Array(
	"assets/styles/trails/trail1.gif",
	32,
	32,
	"assets/styles/trails/trail2.gif",
	24,
	24,
	"assets/styles/trails/trail3.gif",
	20,
	20,
	"assets/styles/trails/trail4.gif",
	16,
	16
);
...
ie5fix2 = 0;
rightedge = B
	? document.documentElement.clientWidth - T1[1]
	: window.innerWidth - T1[1] - 20;
bottomedge = B
	? document.documentElement.scrollTop + document.documentElement.clientHeight - T1[2]
...
for (i=0; i < nos; i++) {
	createContainer(
		...
		i*3+2,
		"",
		"<img src='" + T1[i*3] + "' width="+T1[(i*3+1)]+"px height="+T1[(i*3+2)]+"px border=0>"
	);
}
...
function createContainer(N, Xp, Yp, W, H, At, HT, Op, St){
	with (document){
		write(
			(!A && !B)
				? "<layer id='" + N + "' left=" + Xp + "px top=" + Yp+"px width=" + W + "px height=" + H + "px"
				: "<div id='" + N + "'" + " style='position:absolute;left:" + Xp + "px; top:" + Yp+"px; width:" + W + "px; height:" + H + "px; "
		);
...
function cycle() {
	// do not busy wait
	if (
		(getXpos("CUR"+(nos - 1)) == prevxpos) && 
		(getYpos("CUR"+(nos - 1)) == prevypos)
	) {
...
function newPos(e) {
	moveContainer(
		"CUR"+(nos - 1),
		(B)
			? event.clientX+ie5fix1
			: e.pageX+2,
		(B)
			? event.clientY+ie5fix2
			:e.pageY+2
	);
